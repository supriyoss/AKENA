...
import hashlib, pickle, random, re, string, sys, threading, time
import codecs
import logging
import os
...
def cache_disk(seconds=(86400 * 5), cache_folder="/tmp"):
    ...
    def do_cache(function):
        def inner_function(*args, **kwargs):
            ...
            """
            key = hashlib.sha1((str(args[1]) + str(kwargs)).encode('utf-8')).hexdigest()
...
def get_useragent():
    ...
    """
    with open(settings.USER_AGENTS, 'r') as f:
        agents = f.readlines()
